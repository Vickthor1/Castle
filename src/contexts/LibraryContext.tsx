...
import { getLibraryMeta, setLibraryMeta, type LibraryMeta } from '@/services/libraryStorage'
...
export function LibraryProvider({ children }: { children?: React.ReactNode }) {
  ...
  const refreshApps = useCallback(async () => {
    ...
    setItems((prev) =>
      data.map((it) => {
        ...
        const category = meta.categories.find((c) => c.items.includes(it.id))?.name ?? previous?.category
        return {
          ...it,
  ...
  }, [meta.categories, meta.favorites])
  ...
  const toggleFavorite = (id: string) => {
    const nextFavorites = meta.favorites.includes(id) ? meta.favorites.filter((item) => item !== id) : [...meta.favorites, id]
  ...
  const renameCategory = (id: string, name: string) => {
    ...
    const nextMeta = {
      ...meta,
      categories: meta.categories.map((category) => (category.id === id ? { ...category, name: trimmed } : category))
    }
    persistMeta(nextMeta)
    setItems((prev) => prev.map((item) => (item.category && meta.categories.find((c) => c.id === id)?.name === item.category ? { ...item, category: trimmed } : item)))
  }
  ...
  const deleteCategory = (id: string) => {
    ...
    persistMeta(nextMeta)
    setItems((prev) => prev.map((item) => (item.category === meta.categories.find((c) => c.id === id)?.name ? { ...item, category: undefined } : item)))
  }

  const assignCategory = (id: string, categoryId: string | null) => {
    const category = meta.categories.find((c) => c.id === categoryId)
    const nextCategories = meta.categories.map((entry) => ({ ...entry, items: entry.items.filter((itemId) => itemId !== id) }))
    if (category) {
      nextCategories.find((entry) => entry.id === category.id)?.items.push(id)
    }
    const nextMeta = { ...meta, categories: nextCategories }
    persistMeta(nextMeta)
    setItems((prev) => prev.map((item) => (item.id === id ? { ...item, category: category?.name } : item)))
  }
  ...
  const value = useMemo(
    ...
    [addCategory, assignCategory, clearSelection, deleteCategory, filterCategory, items, meta, openApp, persistMeta, refreshApps, reorderItems, renameCategory, search, selectedIds, setItems, sortBy, toggleFavorite, toggleSelect]
  )

  return <LibraryContext.Provider value={value as LibraryContextValue}>{children}</LibraryContext.Provider>
}

...
export default function LibraryGrid({
  ...
  const effectiveHeight = Math.max(360, height || window.innerHeight - 260);
  ...
  return (
    <div ref={ref} className="h-full w-full min-h-[320px]">
      {items.length === 0 ? (
        <div className="ds-card flex h-full items-center justify-center rounded-[32px] border-dashed border-white/10 bg-[rgba(255,255,255,0.03)] text-sm text-white/60 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
      ...
      {context && (
        <ContextMenu
          ...
          items={[
            ...
            {
              label: context.item.favorite ? 'Remover dos Favoritos' : 'Adicionar aos Favoritos',
              onClick: () => onToggleFavorite?.(context.item),
            },
          ]}
